...
class Dragger {
	...
	private clampTranslate(nextTranslateX: number, nextTranslateY: number) {
		...
		const clampedLeft = Math.min(
			Math.max(left, this._state.minLeft),
			this._state.maxLeft,
		);
		const clampedTop = Math.min(
			Math.max(top, this._state.minTop),
			this._state.maxTop,
		);
	...
	onPointerUp = (e: PointerEvent) => {
		if (
			...
			e.pointerId !== this._state.pointerId
		) {
			return;
		}
...
const bindDrag = (dom: HTMLElement, options?: IDragOptions | null) => {
	...
	addDraggable(
		resolvedOptions.dragElement ?? dom,
		dom,
		resolvedOptions.inViewport,
	);
};

export { bindDrag, addDraggable, removeDraggable, resetDraggableElePosition };
